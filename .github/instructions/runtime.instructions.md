...
**DataFrame formatter**: On kernel start/restart, `_setup_formatters()` silently executes `dataframe_formatter.py` inside the kernel. This registers an IPython MIME formatter for `application/vnd.datateal.dataframe+json` that serialises `pandas.DataFrame` and `duckdb.DuckDBPyRelation` to structured JSON (columns, rows capped at 10 000, row counts).
