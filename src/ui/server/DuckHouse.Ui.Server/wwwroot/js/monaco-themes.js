// Custom Monaco themes for DuckHouse.
// Mirrors VS Code's Light Modern and Dark Modern token colors.
// Uses require() to wait for Monaco AMD modules to load.
// Signals readiness via window._duckhouseThemesReady.
require(['vs/editor/editor.main'], function () {
    'use strict';

    monaco.editor.defineTheme('duckhouse-light', {
        base: 'vs',
        inherit: true,
        colors: {
    ...
    // ── DuckHouse Dark theme (based on VS Code Dark Modern) ──
    monaco.editor.defineTheme('duckhouse-dark', {
        base: 'vs-dark',
        inherit: true,
        colors: {
    ...
    window._duckhouseThemesReady = true;
    if (window._duckhouseThemesResolve) window._duckhouseThemesResolve();
});
