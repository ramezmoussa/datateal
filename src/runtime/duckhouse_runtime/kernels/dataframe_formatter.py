...
Registers a custom formatter for ``application/vnd.duckhouse.dataframe+json``
...
_ROW_CAP = 10_000
MIME_TYPE = "application/vnd.duckhouse.dataframe+json"
...
def register_formatters() -> None:
    """Register the DuckHouse DataFrame MIME formatter with the running IPython kernel."""

    ip = get_ipython()
    if ip is None:
        return

    class _DuckhouseFormatter(BaseFormatter):
        format_type = MIME_TYPE
        print_method = "_repr_duckhouse_"
